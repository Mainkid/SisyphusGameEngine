...
public:
	...
	static void ExtractBoneWeightForVertices(std::vector<DirectX::SimpleMath::Vector4>& vertices, std::unordered_map<std::string, BoneInfo>& boneMap, aiMesh* mesh, const aiScene* scene);

	static std::shared_ptr<Mesh> ProcessMesh(aiMesh* mesh, const aiScene* scene, std::unordered_map<std::string, BoneInfo>& m_BoneInfoMap);
	static void ProcessNode(const std::string& modelPath, std::vector<std::shared_ptr<Mesh>>& mesh, std::unordered_map<std::string, BoneInfo>& m_BoneInfoMap,
	...
	static void LoadModel(const std::string& modelPath, std::vector<std::shared_ptr<Mesh>>& meshes, std::unordered_map<std::string, BoneInfo>& m_BoneInfoMap);
