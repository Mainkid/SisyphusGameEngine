...
int main(int i_iArgC, char* i_pArgV[])
{
    ...
    l_pClient = P7_Create_Client(TM("/P7.Sink=Console"));
    ...
    {
        l_pTrace->P7_TRACE(l_hModule, TM("Test trace message #%d, {%I64d}\n"), l_dwIdx ++, l_qwI);
        l_pTrace->P7_INFO(l_hModule, TM("Test info message #%d, {%I64d}\n"), l_dwIdx ++, l_qwI);
        l_pTrace->P7_DEBUG(l_hModule, TM("Test debug message #%d, {%I64d}\n"), l_dwIdx ++, l_qwI);

        //if (0xF == (rand() & 0xF))
        {
            l_pTrace->P7_WARNING(l_hModule, TM("Test warning message #%d, {%I64d}\n"), l_dwIdx ++, l_qwI);
        }

        //if (0xFF == (rand() & 0xFF))
        {
            l_pTrace->P7_ERROR(l_hModule, TM("Test error message #%d, {%I64d}\n"), l_dwIdx ++, l_qwI);
        }

        //if (0xFF0 == (rand() & 0xFFF))
        {
            l_pTrace->P7_CRITICAL(l_hModule, TM("Test critical message #%d, {%I64d}\n"), l_dwIdx ++, l_qwI);
        }
    }
